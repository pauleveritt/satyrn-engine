...
write absolute paths, start descendants, or mutate shared Git state. E3 proves
delivery with a deterministic executable standing in for a model. E4 adds
writable-path and revision enforcement; E5 adds validation and one real
attempt.
...
The default timeout is 30 seconds. On timeout, the engine terminates and waits
for the direct child. Descendant supervision, cancellation, retry, and repair
are outside E3; its trusted fixtures must not leave descendants running. A
timeout value must be finite and greater than zero; zero, negative, NaN, and
infinite values are CLI `USAGE = 2` errors before delivery starts.
...
The earlier prototype is evidence, not source. Its candidate lifecycle was
inspected at pinned revision
[`ccec5f1`](https://github.com/pauleveritt/local-ai-pi/blob/ccec5f1/harness/candidate.py);
...
2. No-op, nonzero, unavailable, timeout, and moved-`HEAD` commands emit their
   exact receipts, publish no ref, and clean up.
...
- Writable-path and revision enforcement (E4), then validation and a real
  attempt (E5).
- A security sandbox, descendant supervision, retry, repair, or cancellation.
